...
class WAREX3D:
    ...
    def assign(self, amr, task, score, path_len):
        ...
        self.popup(
            f"{self.mode} TASK ASSIGNED",
            f"{amr.name}\n\n"
            f"ORDER: {order}\n"
            f"SHELF: {shelf}\n"
            f"STOP: FRONT OF {shelf}\n"
            f"DESTINATION: {dest}\n"
            f"PRIORITY: {priority}\n\n"
            f"{self.mode} SCORE: {score:.1f}\n"
            f"PATH: {path_len} cells\n"
            f"BATTERY: {amr.battery:.1f}%\n\n"
            f"AMR will stay in the aisle and pick\n"
            f"from the front of the shelf.",
    ...
    def arrival(self, amr):
        ...
        if amr.phase == "SHELF":
            ...
            self.popup(
                "PICK COMPLETE",
                f"{amr.name}\n\nORDER: {order}\n"
                f"ITEM PICKED FROM FRONT OF: {shelf}\n"
                f"NEXT: {dest}\n\n"
                f"Path replanned using obstacle-aware A*.",
                BLUE,
            )
            ...
            self.popup(
                ...
                f"FROM: {shelf}\n"
                f"TO: {dest}\n\n"
                f"Distance: {amr.distance:.1f}\n"
                f"Battery remaining: {amr.battery:.1f}%",
                GREEN,
            )
    ...
    def send_to_charge(self, amr):
        ...
        self.popup(
            "LOW BATTERY — TASK REASSIGNMENT",
            f"{amr.name}\n\n"
            f"Battery: {amr.battery:.1f}%\n"
            f"Emergency action: CHARGE\n"
            f"Station: {charger_name}\n\n"
            f"Task returned to WAREX Brain.\n"
            f"Brain will score the remaining AMRs\n"
            f"and assign the task to another robot.",
            RED,
        )
        self.assign_next_tasks()
    ...
    def finish(self):
        ...
        self.popup(
            "WAREX RUN COMPLETE",
            f"ALL ORDERS COMPLETED\n\n"
            f"Orders: {self.completed}/{len(TASKS)}\n"
            f"Time: {elapsed:.1f} s\n"
            f"Reassignments: {self.total_reassignments}\n\n"
            f"WAREX Brain successfully coordinated\n"
            f"the multi-AMR warehouse.",
            GREEN,
        )
    ...
    def popup(self, title, message, accent):
        win = tk.Toplevel(self.root)
        win.title("WAREX BRAIN")
        win.geometry("420x390")
        win.configure(bg="#0c151d")
        win.resizable(False, False)
        win.transient(self.root)

        tk.Frame(win, bg=accent, height=6).pack(fill="x")

        tk.Label(
            win, text="WAREX AI BRAIN",
            bg="#0c151d", fg=CYAN,
            font=("Segoe UI", 16, "bold")
        ).pack(pady=(18, 2))
        ...
        tk.Label(
            ...
            font=("Consolas", 10),
            justify="left"
        ).pack(fill="both", expand=True, padx=25)

        tk.Button(
            win, text="ACKNOWLEDGE",
            command=win.destroy,
            bg=accent, fg="#061014",
            relief="flat",
            font=("Segoe UI", 10, "bold"),
